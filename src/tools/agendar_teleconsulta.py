...
def agendar_teleconsulta(paciente_id: str, especialidade: str, urgencia: str) -> str:
    ...
    try:
        ...
        print(f"❌ [TOOL ERROR] Falha ao agendar teleconsulta: {str(e)}")
        return json.dumps({
            "erro": f"Falha interna ao tentar agendar consulta: {str(e)}. Informe ao paciente que houve um erro no sistema e peça para tentar novamente mais tarde."
        }, ensure_ascii=False)
